...
export function EmailSavingComponent() {
  ...
  async function sendData() {
    if (isValid && message.length > 1) {
      toast.promise(sendMail({ mail: email, me: message }), {
        ...
        <div className="column is-6">
          ...
          <div className="field has-addons">
            <p className="control is-expanded has-icons-left has-icons-right">
              <input
                className={`input is-rounded ${isValid && "is-success"}`}
                type="text"
            ...
            <p className="control">
              <a className="button is-info" onClick={sendData}>
                {" "}
                Submit{" "}
              </a>
            </p>
          ...
          {renderingBadges[x].map(({ baseLink, custoMoji }, ind) => {
            ...
            return (
              <div className="colum" key={ind}>
                <Link href={`${baseLink}`} className="button">
                  <Image
                    src={custoMoji ?? "/loading.gif"}
                    alt={custoMoji ?? "/loading.gif"}
                    width="100"
                    height="100"
                  />
                </Link>
